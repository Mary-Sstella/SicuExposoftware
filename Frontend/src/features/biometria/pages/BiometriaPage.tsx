...
function BiometriaPage() {
    ...
    useEffect(() => {
        ...
        ws.onopen = () => setEsp32Conectado(true)
        ...
        ws.onmessage = (msg) => {
            ...
            switch (data.evento) {
                ...
                case 'REGISTRO_BD_OK':
                    ...
                    if (estudiante) buscarEstudiante(estudiante.numero_identificacion)
                    break
    ...
    const handleRegistrar = async () => {
        ...
        // Calcular próximo finger_id disponible
    ...
    const handleActualizar = async () => {
        ...
        const finger_id_nuevo = estudiante.finger_id
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex flex-col gap-5">
                    ...
                    <div>
                        ...
                        {errorBusqueda && (
                            <p className="text-xs text-red-500 mt-1.5">{errorBusqueda}</p>
                        )}
                    </div>
                                ...
                                {estudiante.finger_id !== null ? (
                                    <>
                                        ...
                                        <span className="text-xs text-green-600 font-semibold">
                                            Huella registrada — Slot {estudiante.finger_id}
                                        </span>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <div className="bg-red-50 rounded-xl p-3 flex flex-col gap-2">
                                            <div className="flex items-center gap-2 text-red-500">
                                                ...
                                                <span className="text-xs font-semibold">
                                                    ¿Confirmas eliminar la huella de {estudiante.nombres}?
                                                </span>
                                            </div>
                                            <div className="flex gap-2">
                                                <button
                        ...
                        {logs.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                                ...
                                <p className="text-xs text-gray-400">
                                    Los eventos del sensor aparecerán aquí<br />en tiempo real
                                </p>
                            </div>
                        ) : (
                            logs.map((log, i) => (
                                <p key={i} className={`text-xs font-medium ${colorLog(log.tipo)}`}>
                                    {log.texto}
                                </p>
                            ))
                        )}
